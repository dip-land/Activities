...
presence.on('UpdateData', async () => {
  ...
  if (pathname.startsWith('/info')) {
    const infoTitle = document
      ...
      ?.textContent
      ?.trim()
    const infoCover = document.querySelector<HTMLImageElement>(
      'img[src*=\'/file/anilistcdn/media/anime/cover/large\']',
    )?.src

    if (infoTitle)
      presenceData.details = infoTitle
    ...
    const currentAnimeTitle = document.querySelector<HTMLElement>('.anime-title a')?.textContent
    const episodeNumber = document
      .querySelector<HTMLElement>('.ep-number')
      ?.textContent
      ?.match(/\d+/)?.[0]
    const currentAnimeCover = document.querySelector<HTMLImageElement>(
      'img[src*=\'/file/anilistcdn/media/anime/cover/large\']',
    )?.src
    ...
    if (currentAnimeCover)
      presenceData.largeImageKey = currentAnimeCover
